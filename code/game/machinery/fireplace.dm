...
/obj/machinery/fireplace/New()
	..()
	cell = new(src)
	update_icon()

/obj/machinery/fireplace/update_icon()
	overlays.Cut()
	icon_state = "fireplace[on]"
	if(open)
		overlays  += "sheater-open"

/obj/machinery/fireplace/examine(mob/user)
	..(user)
...
/obj/machinery/fireplace/attack_hand(mob/user as mob)
	src.add_fingerprint(user)
	interact(user)

/obj/machinery/fireplace/interact(mob/user as mob)
...
/obj/machinery/fireplace/Topic(href, href_list)
	if (usr.stat)
		return
...
/obj/machinery/fireplace/process()
	if(on)
		if(cell && cell.charge)
...
/obj/machinery/television
	...
	name = "TV"
	desc = "A television for watching shows from all across the galaxy."
	luminosity = 3
	var/on = 0

/obj/machinery/television/attack_hand(mob/user as mob)
	src.add_fingerprint(user)
	interact(user)

/obj/machinery/television/interact(mob/user as mob)
...
/obj/machinery/television/update_icon()
	overlays.Cut()
	icon_state = "security_det[on]"
