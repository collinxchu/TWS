...
/obj/item/weapon/storage/proc/content_can_dump(atom/dest_object, mob/user)
	var/prep = "into" //into vs onto for the display message
	if(Adjacent(user) && dest_object.Adjacent(user))
		//if the moused over atom is NOT a storage item, make sure they want to dump. otherwise try to transfer the contents.
		if (!istype(dest_object, /obj/item/weapon/storage/))
			prep = "onto"
			if("Yes" != alert(usr,"Empty \the [src] onto \the [dest_object]?","Confirm","Yes","No"))
				return
			if(!(usr && dest_object && !usr.stat && !usr.restrained() && usr.canmove))
				return // Something happened while the player was thinking
		if(dest_object.storage_contents_dump_act(src, user))
			usr.face_atom(dest_object)
			usr.visible_message("<span class='notice'>[usr] empties \the [src] [prep] \the [dest_object].</span>",
				"<span class='notice'>You empty \the [src] [prep] \the [dest_object].</span>")
			playsound(loc, "rustle", 50, 1, -5)
			return 1
	return 0
...
/obj/item/weapon/storage/proc/can_be_inserted(obj/item/W as obj, stop_messages = 0, mob/user)
