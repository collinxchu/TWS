...
/datum/reagent/toxin/on_mob_life(mob/living/M, alien)
	current_cycle++
	if(toxpwr)
		M.adjustToxLoss(toxpwr*REM)
	if(alien) return ..() // Kind of a catch-all for aliens without the liver. Because this does not metabolize 'naturally', only removed by the liver.
	return
...
/*	if(iscarbon(M)) #TOREMOVE - refactor xenos, they're horrible
...
/datum/reagent/toxin/plasma/reaction_turf(var/turf/T, var/volume)
	src = null
	T.assume_gas("volatile_fuel", volume, T20C)
	return

/* #TOREMOVE - LINDA atmos
...
datum/reagent/toxin/potassium_chloride/overdose_start(var/mob/living/M as mob)
...
/datum/reagent/toxin/acid/reaction_turf(turf/T, reac_volume)
	...
	for(var/obj/O in T)
		O.acid_act(acidpwr, reac_volume)
/datum/reagent/toxin/acid/pacid
