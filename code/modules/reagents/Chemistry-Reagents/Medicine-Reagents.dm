...
/datum/reagent/medicine/strange_reagent/reaction_mob(mob/living/carbon/human/M, method=TOUCH, reac_volume)
	if(M.stat == DEAD)
		...
		if(!M.suiciding && !(M.disabilities & NOCLONE))
			if(!M)
				return
/*			if(M.notify_ghost_cloning()) #TOREMOVE - implement this
...
datum/reagent/medicine/dexalin/dexalinp
	name = "Dexalin Plus"
	id = "dexalinp"
...
datum/reagent/medicine/dexalin/on_mob_life(var/mob/living/M as mob, var/alien)
...
datum/reagent/medicine/kelotane/on_mob_life(mob/living/M, alien)
...
datum/reagent/medicine/kelotane/overdose_process(mob/living/M)
	M.adjustFireLoss(6*REM) // End result is 3 burn loss taken, because it heals 3 and then removes 6.
	..()
	return
...
datum/reagent/medicine/antitoxin/on_mob_life(mob/living/M, alien)
	if(!alien || alien != IS_DIONA)
...
datum/reagent/medicine/tricordrazine/on_mob_life(mob/living/M, alien)
	if(!alien || alien != IS_DIONA)
		...
		if(M.getBruteLoss() && prob(80)) M.adjustBruteLoss(-1*REM)
		if(M.getFireLoss() && prob(80)) M.adjustFireLoss(-1*REM)
...
datum/reagent/medicine/adrenaline/on_mob_life(var/mob/living/M as mob)
	M.SetParalysis(0)
	M.SetWeakened(0)
...
/datum/reagent/medicine/impedrezene
	name = "Impedrezene"
	id = "impedrezene"
...
/datum/reagent/medicine/impedrezene/on_mob_life(mob/living/M)
...
datum/reagent/medicine/hyronalin/on_mob_life(mob/living/M)
...
datum/reagent/medicine/stoxin/on_mob_life(var/mob/living/M as mob)
	if(!M) M = holder.my_atom
	switch(current_cycle)
		if(1 to 12)
...
datum/reagent/medicine/peridaxon/on_mob_life(var/mob/living/M as mob)
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
