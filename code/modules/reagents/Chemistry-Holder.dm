...
/datum/reagents/proc/metabolize(mob/M, alien, can_overdose = 0)
	if(M)
	...
	for(var/A in reagent_list)
		...
		if(M && R)
			...
			R.on_mob_life(M, alien)	//#TOREMOVE - phase out the unnecessary alien variable--on_mob_life is passed the mob, which should provide more than enough data for whatever we need
	...
	do
		...
		for(var/datum/reagent/R in reagent_list) // Usually a small list
			for(var/reaction in chemical_reactions_list[R.id]) // Was a big list but now it should be smaller since we filtered it with our reagent id
				...
				/*/check if this recipe needs to be heated to mix #TOREMOVE - old heating
...
/datum/reagents/proc/add_reagent(reagent, amount, list/data=null, reagtemp = 300, no_react = 0)
	...
	if(D)
		...
		WARNING("[my_atom] attempted to add a reagent called ' [reagent] ' which doesn't exist. ([usr])")
			...
			/* #TOREMOVE - old code relocated to blood reagent
			// mix dem viruses
