...
/obj/nano_module/alarm_monitor/Topic(ref, href_list)
	...
	//	usr.switch_to_camera(C) #TOREMOVE
		return 1

/obj/nano_module/alarm_monitor/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state /*= default_state  #TOREMOVE */)
	var/data[0]
	...
	for(var/datum/alarm_handler/AH in alarm_handlers)
		...
		/*	if(user.isMobAI())
				for(var/obj/machinery/camera/C in A.cameras())
					cameras[++cameras.len] = C.nano_structure() #TOREMOVE - deal with alarm shit later, it's not important */
...
/*	if (!ui)
		ui = new(user, src, ui_key, "alarm_monitor.tmpl", "Alarm Monitoring Console", 800, 800, state = state) #TOREMOVE
