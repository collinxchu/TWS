...
/*				if("shockwave")
			...
			if("eagles")//SCRAW
				...
				for(var/obj/machinery/door/airlock/W in world)
					if(W.z in config.station_levels && !istype(get_area(W), /area/station/bridge) && !istype(get_area(W), /area/station/crew_quarters) && !istype(get_area(W), /area/station/security/prison))
						W.req_access = list()
