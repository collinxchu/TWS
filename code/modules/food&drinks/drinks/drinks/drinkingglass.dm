	...
	if (reagents.reagent_list.len > 0)
		switch(reagents.get_master_reagent_id())
			...
			if("melonliquor")
				...
				desc = "A sweet and fruity 46 proof liquor."
			if("bluecuracao")
...
//		if(R.glass_center_of_mass)  #TOREMOVE - is this important?
...
/obj/item/weapon/reagent_containers/food/drinks/drinkingglass/attack(obj/target, mob/user)
	if(user.a_intent == "hurt" && ismob(target) && target.reagents && reagents.total_volume)
...
/obj/item/weapon/reagent_containers/food/drinks/drinkingglass/afterattack(obj/target, mob/user, proximity)
	...
	else if(reagents.total_volume && user.a_intent == "hurt")
