...
/obj/machinery/r_n_d/circuit_imprinter/attackby(var/obj/item/O as obj, var/mob/user as mob, params)
	...
	var/amount
	if(stack.amount == 1) //QOL - Don't prompt the user if there's only one sheet in the stack
		amount = 1
	else
		amount = round(input("How many sheets do you want to add?") as num)
	...
	user << "<span class='notice'>You add [amount == 1 ? "the sheet" : "[amount] sheets"] to the [src.name].</span>"
