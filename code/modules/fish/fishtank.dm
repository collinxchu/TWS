...
/obj/machinery/fishtank
	...
	var/is_patched = 0			// 0 if tank is not patched, 1 if it is
...
/obj/machinery/fishtank/proc/check_health()
	...
	if(cur_health <= (max_health * 0.25))			//Major leak at or below 25% health (-10 water/cycle)
		if(is_patched)
			src.visible_message("The duct tape that was holding back the water in \the [src] comes loose! It's leaking pretty badly now.")
			is_patched = 0
		leaking = 2
...
//	T.air_update_turf(1) #TOREMOVE											//Update the air for the turf, to avoid permanent atmos sealing with wall tanks
...
/obj/machinery/fishtank/examine()
	...
	if(water_level > 0)							//Tank has water, so it's actually leaking
		if(is_patched)   examine_message += "\The [src]'s leak is patched up with some duct tape."
		...
		if(is_patched)   examine_message += "\The [src]'s cracks are patched up with some duct tape."
...
/obj/machinery/fishtank/proc/hit(var/damage, var/sound_effect = 1)
	...
	if(is_patched && prob(30))
		src.visible_message("The duct tape that was holding back the water in \the [src] comes loose! It's leaking again.")
		is_patched = 0
	if(sound_effect)
		playsound(loc, 'sound/effects/Glasshit.ogg', 75, 1)
	check_health()

/obj/machinery/fishtank/attack_generic(mob/living/user as mob, damage = 0)	//used by attack_alien, attack_animal, and attack_slime
	user.changeNext_move(CLICK_CD_MELEE)
...
/obj/machinery/fishtank/attackby(var/obj/item/O, var/mob/user as mob)
	...
	//Duct tape will patch minor leaks
	if(istype(O, /obj/item/tape_roll/))
		var/obj/item/tape_roll/W = O
		if(!leaking)
			usr << "There are no cracks to patch on \the [src]."
			return
		else if(leaking == 1 && !is_patched)
			if(W.use(1))
				usr << "You patch up \the [src] with some duct tape. All better!"
				is_patched = 1
				leaking = 0
				check_health()
				return
		else if(leaking == 2)
			usr << "Not even duct tape will save \the [src] now. Maybe try a welder instead."
			return
